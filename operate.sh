...
# === 3. Ensure Python deps (inside venv only) ===
...
"$PYTHON" "$DAEMON_DIR/daemon.py" "$DAEMON_DIR/config.yaml" &
DAEMON_PID=$!

# === 5. Run main.py ===
echo "[INFO] Starting main.py..."
"$PYTHON" "$ROOT_DIR/scripts/main.py" "$DAEMON_DIR/config.yaml" &
MAIN_PID=$!
